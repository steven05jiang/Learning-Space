...
- [~] BUG-013: `cleanup_orphan_tags` CypherSyntaxError on resource deletion — `WITH t.name AS tag_name` projected away `t` before `DELETE t`; fixed by adding `t` to the WITH clause. Fix in `services/graph_service.py:155`. PR pending.
