...
- **Redis**: Upstash (free tier, 500k commands/mo) or self-hosted. URL in Secret. ARQ worker runs in burst mode (exit when queue empty) in production, triggered every 30s via systemd timer. Local dev uses continuous polling. See `docs/queue-enhancement-design.md`.
...
│   └── queue-enhancement-design.md         # Burst mode plan — Upstash free tier + configurable timer
│   ├── ux-requirements.md
│   ├── ux-tech-spec.md
...
| `REDIS_URL` | API, Worker | Upstash (free tier) or self-hosted Redis. ARQ worker uses burst mode in production (exit when queue empty, triggered every 30s via systemd timer). Local dev uses continuous polling. See `docs/queue-enhancement-design.md`. |
