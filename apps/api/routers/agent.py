...
async def query_agent(
    ...
    Returns an AI-generated response based on your resources and the conversation context.
    """
    try:
        response = await agent_service.query(current_user, agent_query)
        return response
    except Exception as e:
        logger.error(f"Error querying agent for user {current_user.id}: {e}", exc_info=True)
        raise HTTPException(
